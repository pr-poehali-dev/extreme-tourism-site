        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <button
              ...
              className="font-montserrat text-sm font-medium text-white/80 hover:text-orange-accent relative group"
            >
              {link.label}
              <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-orange-accent group-hover:w-full" />
            </button>
          ))}
        </nav>
        ...
        <div className="md:hidden bg-[hsl(270,55%,12%)] border-t border-[hsl(270,30%,22%)] px-4 py-6 flex flex-col gap-4">
          {navLinks.map((link) => (
            <button
              key={link.href}
              onClick={() => handleNavClick(link.href)}
              className="font-montserrat text-base font-medium text-white/90 hover:text-orange-accent text-left py-2 border-b border-white/10"
            >
              {link.label}
            </button>
          ))}
